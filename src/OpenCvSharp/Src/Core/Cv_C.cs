...
namespace OpenCvSharp
{
    ...
    public static partial class Cv
    {
            ...
            CvSize winSize, int level, out sbyte[] status, out float[] trackError, CvTermCriteria criteria, LKFlowFlag flags)
        {
            ...
            NativeMethods.cvCalcAffineFlowPyrLK(prev.CvPtr, curr.CvPtr, prevPyr.CvPtr, currPyr.CvPtr, prevFeatures, currFeatures, matrices,
                prevFeatures.Length, winSize, level, status, trackError, criteria, flags);
            KeepAlive(prev, curr, prevPyr, currPyr);
        }
        #endregion
        #region CalcArrHist
#if LANG_JP
        ...
        public static void CalcArrBackProject(CvArr[] image, CvArr backProject, CvHistogram hist)
        {
            ...
            KeepAlive(image, backProject, hist);
        }
#if LANG_JP
        /// <summary>
        /// バックプロジェクションの計算を行う
        ...
        public static void CalcBayesianProb(CvHistogram[] src, CvHistogram[] dst)
        {
            ...
            NativeMethods.cvCalcBayesianProb(srcPtr, src.Length, dstPtr);
            KeepAlive(src, dst);
        }
        #endregion
        ...
        public static void CalcCovarMatrixEx(IplImage[] input, IplImage avg, float[] covarMatrix)
        {
            ...
            using (var handle = ScopedGCHandle.Alloc(inputPtr, GCHandleType.Pinned))
            {
        ...
        public static void CalcEigenObjects(IplImage[] input, IplImage[] output, int ioBufSize, CvTermCriteria calcLimit, IplImage avg, float[] eigVals)
        {
            ...
            {
                NativeMethods.cvCalcEigenObjects(nObjects, inputHandle.AddrOfPinnedObject(), outputHandle.AddrOfPinnedObject(), EigenObjectsIOFlag.NoCallback, 0, IntPtr.Zero, ref calcLimit, avg.CvPtr, eigVals);
            }
            KeepAlive(input, output, avg);
        }
#if LANG_JP
        /// <summary>
        ...
        public static void CalcEigenObjects(int nObjects, IplImage[] input, CvCallback output, int ioBufSize, IntPtr userData, CvTermCriteria calcLimit, IplImage avg, float[] eigVals)
        {
            ...
            using (var inputHandle = ScopedGCHandle.Alloc(inputPtr, GCHandleType.Pinned))
            {
                NativeMethods.cvCalcEigenObjects(nObjects, inputHandle.AddrOfPinnedObject(), output, EigenObjectsIOFlag.OutputCallback, ioBufSize, userData, ref calcLimit, avg.CvPtr, eigVals);
            }
            KeepAlive(input, output, avg);
        }
#if LANG_JP
        ...
        public static void CalcEigenObjects(int nObjects, CvCallback input, IplImage[] output, int ioBufSize, IntPtr userData, CvTermCriteria calcLimit, IplImage avg, float[] eigVals)
        {
            ...
            using (var outputHandle = ScopedGCHandle.Alloc(outputPtr, GCHandleType.Pinned))
            {
                NativeMethods.cvCalcEigenObjects(nObjects, input, outputHandle.AddrOfPinnedObject(), EigenObjectsIOFlag.InputCallback, ioBufSize, userData, ref calcLimit, avg.CvPtr, eigVals);
            }
            KeepAlive(input, output, avg);
        }
#if LANG_JP
        ...
        public static float CalcEMD2(CvArr signature1, CvArr signature2, DistanceType distanceType, CvDistanceFunction distanceFunc, CvArr costMatrix, CvArr flow, ref float lowerBound)
        {
            ...
            {
                var ret = NativeMethods.cvCalcEMD2(
                    signature1.CvPtr, signature2.CvPtr, distanceType, distanceFuncHandle.AddrOfPinnedObject(), ToPtr(costMatrix), ToPtr(flow), ref lowerBound, 0);
                KeepAlive(signature1, signature2, distanceFunc, costMatrix, flow);
                return ret;
            }
        }
        #endregion
        ...
        public static void CalcImageHomography(float[] line, CvPoint3D32f center, float[,] intrinsic, out float[,] homography)
        {
            ...
            NativeMethods.cvCalcImageHomography(line, ref center, intrinsic, homography);
        }
        #endregion
        #region CalcHist
#if LANG_JP
...
// ReSharper restore CoVariantArrayConversion
            ...
            CvSize winSize, int level, out sbyte[] status, out float[] trackError, CvTermCriteria criteria, LKFlowFlag flags)
        {
            ...
            NativeMethods.cvCalcOpticalFlowPyrLK(prev.CvPtr, curr.CvPtr, ToPtr(prevPyr), ToPtr(currPyr), prevFeatures, currFeatures, prevFeatures.Length, winSize, level, status, trackError, criteria, flags);
            KeepAlive(prev, curr, prevPyr, currPyr, prevFeatures);
        }
        #endregion
        #region CalcPCA
#if LANG_JP
...
// ReSharper restore InconsistentNaming
        ...
        public static int CamShift(CvArr probImage, CvRect window, CvTermCriteria criteria, out CvConnectedComp comp, out CvBox2D box)
        {
            ...
            box = new CvBox2D();
            var ret = NativeMethods.cvCamShift(probImage.CvPtr, window, criteria, comp.CvPtr, ref box);
        ...
        {
            return NativeMethods.cvCheckTermCriteria(criteria, defaultEps, defaultMaxIters);
        }
        #endregion
        #region Circle
#if LANG_JP
        ...
        {
            return NativeMethods.cvClipLine(imgSize, ref pt1, ref pt2);
        }
        #endregion
        #region Clone
#if LANG_JP
        ...
        {
            return NativeMethods.cvClone(structPtr);
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトのコピーを作成する
        ...
        {
            NativeMethods.cvConDensInitSampleSet(condens.CvPtr, lowerBound.CvPtr, upperBound.CvPtr);
            KeepAlive(condens, lowerBound, upperBound);
        }
        #endregion
        ...
        {
            NativeMethods.cvConDensUpdateByTime(condens.CvPtr);
            GC.KeepAlive(condens);
        }
        #endregion
        #region CompleteSymm
#if LANG_JP
...
// ReSharper disable InconsistentNaming
                 ...
                 CvMat dr3dr1, CvMat dr3dt1, CvMat dr3dr2, CvMat dr3dt2, CvMat dt3dr1, CvMat dt3dt1, CvMat dt3dr2, CvMat dt3dt2)
        {
            ...
            KeepAlive(rvec1, tvec1, rvec2, tvec2, rvec3, tvec3,
...
// ReSharper restore InconsistentNaming
        ...
        public static void ComputeCorrespondEpilines(CvMat points, int whichImage, CvMat fundamentalMatrix, out CvMat correspondentLines)
        {
            ...
            KeepAlive(points, fundamentalMatrix);
        }
        #endregion
        #region ContourArea
#if LANG_JP
        ...
        public static void ConvexHull2(CvArr input, out int[] hull, ConvexHullOrientation orientation)
        {
            ...
            {
                NativeMethods.cvConvexHull2(input.CvPtr, hullMat.CvPtr, orientation, 0);
        ...
        public static void ConvexHull2(CvArr input, out CvPoint[] hull, ConvexHullOrientation orientation)
        {
            ...
            {
                NativeMethods.cvConvexHull2(input.CvPtr, hullMat.CvPtr, orientation, 1);
        ...
        public static void CopyHist(CvHistogram src, ref CvHistogram dst)
        {
            ...
            if (dst == null)
            {
                ...
                NativeMethods.cvCopyHist(src.CvPtr, ref dstPtr);
        ...
        public static void CorrectMatches(CvMat f, CvMat points1, CvMat points2, CvMat newPoints1, CvMat newPoints2)
        {
            ...
            KeepAlive(f, points1, points2, newPoints1, newPoints2);
        }
        #endregion
        #region CountNonZero
#if LANG_JP
        ...
        {
            IntPtr ptr = NativeMethods.cvCreateBGCodeBookModel();
            if (ptr == IntPtr.Zero)
                return null;
        ...
        public static CvGraphScanner CreateGraphScanner(CvGraph graph, CvGraphVtx vtx, GraphScannerMask mask)
        {
            ...
            return new CvGraphScanner(NativeMethods.cvCreateGraphScanner(graph.CvPtr, vtxPtr, mask));
        }
        #endregion
        #region CreateImage
#if LANG_JP
        ...
        {
            IntPtr ptr = NativeMethods.cvCreateImage(size, depth, channels);
            if (ptr == IntPtr.Zero)
                return null;
            return new IplImage(ptr);
        }
        ...
        {
            IntPtr ptr = NativeMethods.cvCreateImageHeader(size, depth, channels);
            if (ptr == IntPtr.Zero)
                return null;
            return new IplImage(ptr);
        }
        ...
        {
            IntPtr ptr = NativeMethods.cvCreateLSH(ops, d, l, k, type, r, seed);
            if (ptr == IntPtr.Zero)
                return null;
        ...
        {
            IntPtr ptr = NativeMethods.cvCreateMat(rows, cols, type);
            if (ptr == IntPtr.Zero)
                return null;
            return new CvMat(ptr);
        }
        ...
        {
            IntPtr ptr = NativeMethods.cvCreateMemoryLSH(d, n, l, k, type, r, seed);
            if (ptr == IntPtr.Zero)
                return null;
            return new CvLSH(ptr);
        }
        ...
        public static T[] CvtSeqToArray<T>(CvSeq seq, out T[] elements, CvSlice slice) where T : struct
        {
            ...
            //if (typeof(t).IsValueType)  // where t : struct
            {
                ...
                {
                    NativeMethods.cvCvtSeqToArray(seq.CvPtr, elementsPtr, slice);
                }
            }
            /*
